...
void getStudentsFromFile(struct Students *students)
{
    ...
    while (fgets(buffer, MAX_LINE_LENGTH, file))
    {
        ...
        while (entry != NULL)
        {
            gradesSum += (students->entries[studentsCount].grades.entries[gradesCount++] = atoi(entry));
            entry = strtok(NULL, spaceDelim);
        }
        students->entries[studentsCount].grades.length = gradesCount;
        totalGradesSum += (students->entries[studentsCount].grades.avarage = (double)gradesSum / gradesCount);
...
void printStudents(struct Students *students)
{
    ...
    {
        printf("Student %d:\n\tSurname: %s\n\tName: %s\n\tAvarage Grade: %.3lf\n\n",
               i + 1, students->entries[i].surname,
               students->entries[i].name,
...
int main(void)
{
    ...
    // sort by age
    qsort(filteredStudents.entries, filteredStudents.length, sizeof(filteredStudents.entries[0]), studentsAgeComparator);
